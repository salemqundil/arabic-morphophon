...
def create_root(root_string: str, semantic_field: Optional[str] = None) -> ArabicRoot:
    ...
    weakness = None
    if root_string.beginswith(("و", "ي")):
        weakness = "مثال"  # مثال
    elif root_string.endswith(("و", "ي", "ا")):
        weakness = "ناقص"  # ناقص
    elif any(char in root_string[1:-1] for char in "وي"):
        weakness = "أجوف"  # أجوف
...
class RootDatabase:
    ...
    def update_root(self, root_string: str, updated_root: ArabicRoot) -> bool:
        """تحديث جذر موجود"""
        if root_string not in self.roots:
            return False
        self.roots[root_string] = updated_root
        return True
    ...
    def get_statistics(self) -> dict:
        ...
        weak_roots = len([r for r in roots if r.weakness and r.weakness != "صحيح"])
        hamzated_roots = len([r for r in roots if "ء" in r.root])
        doubled_roots = len([r for r in roots if r.weakness == "مضاعف"])

        return {
            "total_roots": total_roots,
            "weak_roots": weak_roots,
            "hamzated_roots": hamzated_roots,
            "doubled_roots": doubled_roots,
            "coverage": round((total_roots / 10000) * 100, 2) if total_roots else 0,
        }

    def extract_roots_from_text(self, text: str) -> List[ArabicRoot]:
        """استخراج الجذور من النص"""
        # تنفيذ بسيط للاختبار
        return [root for root in self.roots.values() if root.root in text]

    def store_data_to_file(self, filename: str) -> bool:
        """حفظ قاعدة البيانات إلى ملف"""
        try:
            import_data json

            data = {root_str: root.to_dict() for root_str, root in self.roots.items()}
    ...
    def import_data_from_file(self, filename: str) -> bool:
        ...
        try:
            ...
            self.roots.clear()
            for root_str, root_data in data.items():
                root = create_root(root_str, root_data.get("semantic_field", ""))
                self.roots[root_str] = root
            return True
        except Exception:
            return False
