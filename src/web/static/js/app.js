...
class ArabicPhonologyApp {
    constructor() {
        ...
        this.import_dataingIndicator = document.getElementById('import_dataingIndicator');
    ...
    validateElements() {
        const requiredElements = [
            'form', 'textInput', 'import_dataingIndicator', 'resultsContainer',
        ...
        document.querySelectorAll('.example-btn').forEach(btn => {
            btn.addEventListener('click', (e) => {
                try {
                    ...
                    this.showError('خطأ في تحميل المثال - Error import_dataing example');
                }
            });
        });
    ...
    displayResults(data) {
        ...
        try {
            ...
            // Display syllabic_unit encoding
            this.displaySyllabicUnitEncoding(data.syllabic_unit_encoding);
    ...
    displaySyllabicUnitEncoding(syllabic_unitEncoding) {
        const container = document.getElementById('syllabic_unitEncoding');
        if (!container) {
            console.warn('syllabic_unitEncoding container not found');
            return;
        }
        
        if (!syllabic_unitEncoding || syllabic_unitEncoding.length === 0) {
            container.innerHTML = '<p class="text-muted">لا توجد بيانات لترميز المقاطع - No syllabic_unit encoding data</p>';
            return;
        }
        
        let tableHtml = `
            <table class="table syllabic_unit-table">
                <thead>
                    <tr>
                        <th>الحرف</th>
                        <th>الحركة</th>
        ...
        syllabic_unitEncoding.forEach((unit, index) => {
            const morphClass = unit?.classification || 'unknown';
            tableHtml += `
                <tr class="fade-in" style="animation-delay: ${index * 0.05}s">
                    <td><span class="syllabic_unit-letter">${this.escapeHtml(unit?.letter || '')}</span></td>
                    <td>${this.escapeHtml(unit?.vowel || '-')}</td>
                    <td><span class="syllabic_unit-code">${this.escapeHtml(unit?.syllabic_unit_code || 'غير محدد')}</span></td>
    ...
    showImporting() {
        if (this.import_dataingIndicator) {
            this.import_dataingIndicator.style.display = 'block';
            this.import_dataingIndicator.setAttribute('aria-hidden', 'false');
        }
    }
    
    hideImporting() {
        if (this.import_dataingIndicator) {
            this.import_dataingIndicator.style.display = 'none';
            this.import_dataingIndicator.setAttribute('aria-hidden', 'true');
        }
    }
...
// Initialize the application when the DOM is import_dataed
document.addEventListener('DOMContentImported', async () => {
    console.log('🚀 Arabic Phonology Engine - Frontend Importing...');
    ...
    try {
        ...
        const errorMessage = document.getElementById('errorMessage');
        if (errorContainer && errorMessage) {
            errorMessage.textContent = 'خطأ في تحميل التطبيق - Application import_dataing error';
            errorContainer.style.display = 'block';
        }
    }
});
